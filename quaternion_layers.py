...
class QuaternionConv(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
                 dilatation=1, padding=1, groups=1, bias=True, init_criterion='glorot',
                 weight_init='quaternion', seed=None, operation='convolution2d', rotation=True, quaternion_format=True, scale=False):
    ...
    def forward(self, input):
        ...
        if self.rotation:
            ...
            return quaternion_conv(input, self.r_weight, self.i_weight, self.j_weight, self.k_weight, self.bias, self.stride, self.padding, self.groups, self.dilatation)
...
if  __name__ == "__main__":
    dc = QuaternionConv(in_channels=4, out_channels=16, kernel_size=3, stride=1)
    A = torch.FloatTensor(size=(1, 1, 256, 256)).normal_(0, 1)
    B = torch.FloatTensor(size=(1, 4, 256, 256)).normal_(0, 1)
    out = dc(B)
    print(out.shape)